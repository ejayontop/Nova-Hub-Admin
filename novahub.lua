local Config = {
    ServiceId       = 27652, 
    PlatoSecret     = "8e618504-6630-46c3-8ad9-628e319316ff", 
    Secret          = "roseteejay@0309",
    MainScriptURL   = "https://raw.githubusercontent.com/NovaHubRBLX/NovaHub/refs/heads/main/loader.lua", 
    PremiumKeysURL = "https://raw.githubusercontent.com/ejayontop/key/refs/heads/main/key.txt",
    ShowDiscord     = true,
    DiscordURL      = "https://discord.gg/JEGzF76nW",
    ShowInstagram   = false,
    InstagramURL    = "",
    ShowYoutube     = false,
    YoutubeURL      = "",
    KeyFileName     = "novahubKey.txt", 
    ...
    HubName         = "Nova Hub", -- The main title shown at the top of the GUI
    HubDescription  = "🎉 Join Our Discord & Claim Your FREE 7-Day Premium Key!" -- The text shown below the title
}
...
local function checkPremiumKey(inputKey)
    local success, response = pcall(function() return game:HttpGet(Config.PremiumKeysURL) end)
    if not success then return false, "Could not reach server" end
    
    for keyStr, expiryStr in string.gmatch(response, "%[(.-),%s*(.-)%]") do
        if keyStr == inputKey then
...
local function CreateGUI()
    ...
    -- Clean Vibe Stroke Function (No rainbow cycle, fits dark theme beautifully)
    local function AddAccentedStroke(parent)
